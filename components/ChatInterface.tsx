...
function ChatInterface({ chatId, initialMessages }: ChatInterfaceProps) {
    ...
    const formatTerminalOutput = (tool: string, input: unknown, output: unknown): string => {
        const terminalHtml = `
            <div class="bg-[#333] text-white font-mono p-2 rounded-md my-2 overflow-x-auto whitespace-normal max-w-[600px]">
                ...
                <div class="text-gray-400 mt-1">$ Output</div>
                <pre class="text-yellow-400 mt-0.5 whitespace-pre-wrap overflow-x-auto">${formatToolOutput(output)}</pre>
            </div>`;
        return terminalHtml;
    };
  ...
  // Initialize SSE connection 
            ...
            setStreamedResponse("Error occurred while processing your message.");
        } finally {
            setIsLoading(false);
        }
    };
        ...
        try {
            ...
            await processStream(reader, async (chunk) => {
                ...
                for (const message of messages) {
                    switch(message.type){
                        case StreamMessageType.Token:
                            if (message.type === StreamMessageType.Token && "token" in message) {
                                fullResponse += message.token;
                                setStreamedResponse(fullResponse);     
                    }
                    break;
...
{streamedResponse && (
  <MessageBubble 
    ...
    isUser={true} // or false, depending on the message origin
  />
)}
